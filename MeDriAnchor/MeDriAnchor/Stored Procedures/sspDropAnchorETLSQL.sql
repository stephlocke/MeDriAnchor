...
CREATE PROCEDURE [MeDriAnchor].[sspDropAnchorETLSQL]
(
	@AnchorName SYSNAME,
...
BEGIN TRY
	...
	-- generate the drop
	SET @SQL += 'IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N''[' + @encapsulation + '].[' + @ProcNamePrefix + @AnchorName + @ProcNamePostfix + ']'') AND type in (N''P'', N''PC''))' + char(13)
	SET @SQL += 'DROP PROCEDURE [' + @encapsulation + '].[' + @ProcNamePrefix + @AnchorName + @ProcNamePostfix + '];' + CHAR(13) + CHAR(13);
