CREATE PROCEDURE [MeDriAnchor].[sspCreateKnotETLSQL]
(
	@KnotName SYSNAME,
	@Batch_ID BIGINT,
	@Metadata_ID BIGINT,
...
SET NOCOUNT ON;
/*
GENERATES THE SQL FOR THE CREATE OF A KNOT ETL SQL PROCEDURE
*/
DECLARE @SQL NVARCHAR(MAX) = '';
DECLARE @ProcNamePrefix NVARCHAR(20) = 'amsp_Knot_';
...
BEGIN TRY
	...
	SELECT	@ProcNamePrefix = [KnotETLProcNamePrefix],
	...
	WHERE [Metadata_ID] = @Metadata_ID
		...
		AND [DWHType] = 'Knot'
		AND [DWHName] = @KnotName;

	SET @ProcName = '[' + @encapsulation + '].[' + @ProcNamePrefix + @KnotName + @ProcNamePostfix + ']';

	-- generate the run procedure
	IF NOT EXISTS(SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_SCHEMA = @encapsulation
		AND ROUTINE_NAME = @ProcNamePrefix + @KnotName + @ProcNamePostfix)
	BEGIN
		SET @SQL += 'CREATE PROC [' + @encapsulation + '].[' + @ProcNamePrefix + @KnotName + @ProcNamePostfix + ']' + CHAR(13);
	END
	ELSE
	BEGIN
			SET @SQL += 'ALTER PROC [' + @encapsulation + '].[' + @ProcNamePrefix + @KnotName + @ProcNamePostfix + ']' + CHAR(13);
	END
	SET @SQL += '(' + CHAR(13);
	...
	SET @SQL += '				ETL for knot ' + @KnotName + CHAR(13);
	SET @SQL += ' - code:	EXEC [' + @encapsulation + '].[' + @ProcNamePrefix + @KnotName + @ProcNamePostfix + ']' + CHAR(13);
	...
	SET @SQL += CHAR(9) + 'VALUES(@Batch_ID, 1, ''Completed ETL for Knot ' + @KnotName + ''', @RecordsInserted);' + CHAR(13) + CHAR(13);
	...
	SET @SQL += CHAR(9) + 'VALUES(@Batch_ID, 4, ''Error with ETL for Knot ' + @KnotName + '. Error: '' + @ErrorMessage);' + CHAR(13) + CHAR(13);
