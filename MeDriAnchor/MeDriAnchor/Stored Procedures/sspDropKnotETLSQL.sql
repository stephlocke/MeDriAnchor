...
CREATE PROCEDURE [MeDriAnchor].[sspDropKnotETLSQL]
(
	@KnotName SYSNAME,
...
AS
/*
GENERATES THE SQL FOR THE DROP OF A KNOT ETL SQL PROCEDURE
*/
...
BEGIN TRY
	...
	-- generate the drop
	SET @SQL += 'IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N''[' + @encapsulation + '].[' + @ProcNamePrefix + @KnotName + @ProcNamePostfix + ']'') AND type in (N''P'', N''PC''))' + char(13)
	SET @SQL += 'DROP PROCEDURE [' + @encapsulation + '].[' + @ProcNamePrefix + @KnotName + @ProcNamePostfix + '];' + CHAR(13) + CHAR(13);
