...
function runCROWETL($medrianchordbname, $medrianchordbserver, $environment, $debug)
{
    ...
    #########################################################################################################################
    ## STEP 1: CHECK THAT WE ARE RUNNING AS THE CORRECT USER (IF NEEDED)
    #########################################################################################################################

    write-host "Checking for the correct running user...." -ForegroundColor Yellow;

    #$runninguser = [Environment]::UserName
    #if ($runninguser.CompareTo("{whatever}") = 0)
    #{
    #    $errorMsg = "This must be run as the {whatever} windows user. Stopping.";
	#    throw $errorMsg;
    #}
    ...
    [string]$initiating = "Initiating batch (" + $environment + ")...."
   ...
   write-host "Initiated batch...." -ForegroundColor Yellow;
    ...
    if ($metadataChanged -eq 1) # only do if we have changed metadata
    {
        ...
        try
        {
            ...
            $xml.Save("C:\anchormodeler\0.98\LatestAnchorXML_" + $environmentschema + ".xml");
        ...
        try
        {
			...
			[string]$xmlanchorfile = "LatestAnchorXML_" + $environmentschema + ".xml";
			[string]$xmlanchorsql = "LatestDWHSQL_" + $environmentschema + ".sql";
            cscript.exe "Sisulator.js" //nologo -x $xmlanchorfile -m Anchor -d "SQLServer_uni.directive" -o $xmlanchorsql;
		...
		$xmlanchorsqltorun = "C:\anchormodeler\0.98\" + $xmlanchorsql;
        $dwhScriptFile = Get-Item $xmlanchorsqltorun;
        ...
        try
        {
			...
			$SqlCmdMaint3.Parameters.Add("@Environment_ID", [System.Data.SqlDbType]"SmallInt") >> $null;
            $SqlCmdMaint3.Parameters["@Environment_ID"].Value = [Int32]$environmentid;
...
runCROWETL "MeDriAnchor" "TECHNOBITCH" "PRODUCTION" 0;
runCROWETL "MeDriAnchor" "TECHNOBITCH" "UAT" 0;
runCROWETL "MeDriAnchor" "TECHNOBITCH" "DEVELOPMENT" 0;